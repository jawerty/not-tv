...
class ChannelProgramming {
	...
	createTimeSlots(timebox) { // timebox is int of hours (24 for 24 hours)
		...
		const timeboxInMiliSeconds = (timebox * 60 * 60) * 1000;
		...
		let cursor = 0;
		let i = 0;
		console.log("TOTAL_TIME:", timeboxInMiliSeconds)
		while (timeboxDuration < timeboxInMiliSeconds) {
			// console.log("TIME:", timeboxDuration)
			// check if current time is in new hour
			const cursorTime = new Date(new Date(d.getTime() + (timeboxDuration)).toLocaleString());
			// console.log(cursorTime.getTime(), cursorTime.toUTCString())
			...
			timeboxDuration += (channelVideo.duration * 1000);
			channelVideo.video_start_ts = cursorTime.getTime();
			timeslots.push(Object.assign({}, channelVideo));
