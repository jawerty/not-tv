...
class CookieStore {
	constructor() {
		this.cookies = [];
		this.cookieJar = {};
		this.cookieFilePath = path.join(__dirname, "../data", "cookies.json");
	}
	...
	saveCookieJSON(cookieJSON) {
		return new Promise((resolve, reject) => {
			fs.writeFile(this.cookieFilePath, JSON.stringify(cookieJSON), 'utf8', (err) => {
				if (err) throw err;
				resolve(true);
			});
		});
	...
	fetchCookieJSON() {
		return new Promise((resolve, reject) => {
			fs.readFile(this.cookieFilePath, (err, data) => {
			    if (err) throw err;
